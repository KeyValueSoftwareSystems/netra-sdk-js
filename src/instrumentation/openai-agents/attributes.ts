...
export function setSpanDataAttributes(
  otelSpan: OTelSpan,
  agentSpan: AgentSpan,
  systemName: string = DEFAULT_LLM_SYSTEM,
): void {
  ...
  switch (data.type) {
    case "agent":
      ...
      break;
    case "generation":
      setGenerationAttributes(otelSpan, data, systemName);
      break;
    case "response":
      setResponseAttributes(otelSpan, agentSpan, systemName);
      break;
    case "function":
...
function setResponseAttributes(span: OTelSpan, agentSpan: AgentSpan, systemName: string): void {
  const data = agentSpan.spanData;
  span.setAttribute(SpanAttributes.LLM_SYSTEM, systemName);
...
function setIndexedPromptAttributes(span: OTelSpan, input: unknown, startIndex = 0): void {
  ...
  for (const message of input) {
    if (index - startIndex >= MAX_INDEXED_MESSAGES) break;
...
function setIndexedCompletionAttributes(span: OTelSpan, responseData: unknown): void {
  ...
  if (Array.isArray(resp.output)) {
    ...
    for (const item of resp.output) {
      if (index >= MAX_INDEXED_MESSAGES) break;
  ...
  if (Array.isArray(resp.choices)) {
    ...
    for (const choice of resp.choices) {
      ...
      if (c.finish_reason) {
        span.setAttribute(`${SpanAttributes.LLM_COMPLETIONS}.${choiceStartIndex}.finish_reason`, String(c.finish_reason));
      }
    }
  }
}
